...
#define PATH_COLLISION_FACTOR (5)
#define EXPAND_COLLISION_LIMIT (150)
...
tyPolygon* AStarPathfinder::generatePathArea(LocationWWeight start,
		LocationWWeight end, float theta)
{
...
tyPolygon* AStarPathfinder::generatePathArea(float xStart, float yStart,
		float xEnd, float yEnd, float theta)
{
	tyPolygon *pathArea = new tyPolygon;
	LocationWWeight helper;

	//compute start points
	helper.x = xStart + (-pRobot->getLengthMm() / 2) * cos(theta)
			- (+(pRobot->getWidthMm() + 30) / 2) * sin(theta);
	helper.y = yStart + (-pRobot->getLengthMm() / 2) * sin(theta)
			+ (+(pRobot->getWidthMm() + 30) / 2) * cos(theta);
	pathArea->push_back(helper);
	...
	//compute end points
	helper.x = xEnd + (+pRobot->getLengthMm() / 2) * cos(theta)
			- (-(pRobot->getWidthMm() + 30) / 2) * sin(theta);
	helper.y = yEnd + (+pRobot->getLengthMm() / 2) * sin(theta)
			+ (-(pRobot->getWidthMm() + 30) / 2) * cos(theta);
	pathArea->push_back(helper);
...
bool AStarPathfinder::checkForAlreadyThere(PathNode *origin, PathNode *current,
		float xnew, float ynew)
{
	...
	for (list<PathNode *>::iterator it = origin->children.begin();
			it != origin->children.end(); ++it)
	{
		if ((*it != current)
				&& euclidDistance((*it)->x, (*it)->y, xnew, ynew)
						< EXPAND_COLLISION_LIMIT)
			return true;
	}
	...
	{
		if ((*it != current)
				&& checkForAlreadyThere((*it), current, xnew, ynew) == true)
			return true;
	}
...
void AStarPathfinder::expandOnDirection(PathNode* origin, float rotation,
		const LocationWWeight& target, PathNode* current)
{
	...
	if (insidePolygon2D(target, *pathArea))
	{
		...
		y = target.y;
		cost = current->cost
				+ euclidDistance(current->x, current->y, target.x, target.y)
						* COST_MOVEMENT;
		heuristic = 0;
		delete pathArea;
		pathArea = generatePathArea(current->x, current->y, x, y, theta);
	...
	if (world)
	{
		if (world->computeCollisionFactor(pathArea) < PATH_COLLISION_FACTOR)
		{
			if (checkForAlreadyThere(origin, current, x, y) == false)
				current->children.push_back(
						new PathNode(current, x, y, theta, cost, heuristic));
		}
	}
	else
	{
...
void AStarPathfinder::expandPathNode(PathNode* origin,
		const LocationWWeight& target, PathNode* current)
{
	//cout << "expanding " << current << " chld: " << current->children.size() <<  endl;
	//optimize to make sure we get straight path
	float posToTargetAngle = atan2((target.y - current->y),
			(target.x - current->x));
	for (float rotation = 0; rotation < M_PI; rotation += STEP_ROTATION)
	{
		...
		{
			//positive rotation
			expandOnDirection(origin, posToTargetAngle - rotation, target,
					current);
		}
	}
	current->closed = true;
...
bool AStarPathfinder::arrivedAtGoal(const LocationWWeight& target,
		PathNode* currentNode)
{
	float distance = euclidDistance(currentNode->x, currentNode->y, target.x,
			target.y);
...
bool AStarPathfinder::checkPathForMapUpdates(PathNode* path)
{
	...
	else
	{
		tyPolygon * area = generatePathArea(path->x, path->y,
				(*path->children.begin())->x, (*path->children.begin())->y,
				path->theta);

		if (world->computeCollisionFactor(area) < PATH_COLLISION_FACTOR)
		{
			delete area;
...
void AStarPathfinder::addEachNodeToProtobuf(PathNode* originReal,
		robotdata::FullPath_PathDot *originProto)
{
	if (originReal == NULL)
		return;
	if (originProto == NULL)
	...
	//search all children for collision
	for (list<PathNode *>::iterator it = originReal->children.begin();
			it != originReal->children.end(); ++it)
	{
...
google::protobuf::Message * AStarPathfinder::sendPath(PathNode * originReal)
{
	...
	robotdata::FullPath_PathDot *originProto =
			new robotdata::FullPath_PathDot();
